...
// Simple in-memory cache for API calls during build
const apiCache = new Map();

/**
 * Cached fetch wrapper to avoid duplicate API calls during the same build
...
module.exports = async function() {
  ...
  try {
    ...
    let response;
    try {
      response = await cachedFetch(basicApiUrl, { 
        timeout: 10000 
      });
    } catch (e) {
        ...
        richContentComponents = await Promise.all(fullPost.rich_content.map(async (component, index) => {
          ...
          switch (component.__component) {
              ...
              if (component.body) {
                ...
                if (mediaDocumentIds.length > 0) {
                  ...
                  try {
                    ...
                    const mediaResponse = await cachedFetch(mediaApiUrl, { timeout: 5000 });
            ...
            case 'book-flip':
              ...
              // Skip PDF processing in development mode for faster builds
              const isDev = process.env.ELEVENTY_ENV === 'development';
              
              if (!isDev && component.pdf_file?.url) {
                try {
              ...
              } else if (isDev && component.pdf_file?.url) {
                console.log(`⚡ Skipping PDF processing in dev mode for: ${component.title || 'Book Flip'}`);
              }
              ...
              return {
                ...
                error: pages.length === 0 && !isDev ? 'No pages processed' : null,
                devMode: isDev
              };
